...
public class IllustrationService {
    ...
    public void saveToDbSync(List<Illustration> illustrations) {
        ...
        illustrations.stream().parallel().forEach(e -> {
            if ((e.getTotalBookmarks() < 200 || reviewFilter.filter(e.getTitle() + e.getCaption()) || e.getTags().stream().anyMatch(t -> reviewFilter.filter(t.getName()) || reviewFilter.filter(t.getTranslatedName()))) && e.getSanityLevel() < 7) {
                e.setSanityLevel(7);
            }
        });
