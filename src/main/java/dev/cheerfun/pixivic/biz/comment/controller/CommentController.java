...
public class CommentController {
    ...
    public ResponseEntity<Result<String>> pullComment(@PathVariable String commentAppType, @PathVariable int commentAppId, @RequestBody @SensitiveCheck Comment comment, @RequestHeader("Authorization") String token) {
        comment.init(commentAppType, commentAppId, (int) AppContext.get().get(USER_ID));
        commentService.pushComment(comment);
        //拼接是否点赞

        return ResponseEntity.ok().body(new Result<>("拉取评论成功"));
    }
